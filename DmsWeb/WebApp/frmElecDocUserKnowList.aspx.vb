...
Partial Class WebApp_frmElecDocUserKnowList
    ...
    Private Sub doc_query()
        ...
        sqltmp += "'~/WebApp/frmElecDocFormBookOut.aspx?docformid=' + rtrim(convert(char,doc_type)) + '&rowid=' + rtrim(convert(char,t1.id)) + "
        sqltmp += " case doc_approved when '1' then '&approved_mode=yes' else '&approved_mode=no' end as url_field,"
        sqltmp += " case when doc_status=0 and t1.doc_obj_to=1 then 'รอตรวจสอบ' "
        sqltmp += " when doc_status=1 then 'ยังไม่ได้ส่ง' "
        sqltmp += " when doc_status=2 then 'ส่งแล้ว' "
        sqltmp += " when doc_status=3 then 'อนุมัติ' "
        sqltmp += " when doc_status=4 then 'ไม่อนุมัติ' "
        sqltmp += " when doc_status=5 then 'ส่งกลับเพื่อแก้ไข' "
        ...
        sqltmp += " where 1=1 "
        If txtDate1.DateValue.Year <> 1 Then sqltmp = sqltmp + " and doc_send_date >= '" + Format(DateAdd(DateInterval.Year, YearZoneSave, txtDate1.DateValue), "yyyy/MM/dd") + "'"
        If txtDate2.DateValue.Year <> 1 Then sqltmp = sqltmp + " and doc_send_date <= '" + Format(DateAdd(DateInterval.Year, YearZoneSave, txtDate2.DateValue), "yyyy/MM/dd") + "'"
        If Trim(txttitle.Text) <> "" Then sqltmp = sqltmp + " and doc_title like '%" + Trim(txttitle.Text) + "%'"
        If chkNoRead.Checked = True Then sqltmp += " and t1.is_read='N'"
        sqltmp = sqltmp + " and doc_to = '" + Config.GetLogOnUser.OFFICER_DATA.ID.ToString + "'"
        sqltmp = sqltmp + " and ((doc_obj_to = '" & Para.Common.Utilities.Constant.DocObjective.OjbKnow & "' "
        sqltmp = sqltmp + " and doc_status in ('" & cfg_docstart & "','" & cfg_docnoapproveedit & "'))"
        sqltmp = sqltmp + " or (doc_obj_to = '" & Para.Common.Utilities.Constant.DocObjective.ObjApprove & "' and doc_status = '" & cfg_docnoapprove & "'))"
        ...
        If Ds.Tables("DATA").Rows.Count > 0 Then
            For i As Integer = 0 To Ds.Tables("DATA").Rows.Count - 1
                Ds.Tables("DATA").Rows(i)("rownum") = (i + 1).ToString
            Next
    ...
    Private Sub approver_doc(ByVal status As String)
        ...
        For r = 0 To GridView1.Rows.Count - 1
            ...
            If isChecked.Checked Then
                ...
                sqltmp = sqltmp + ",'0'"
                sqltmp = sqltmp + ",[doc_OwnerAddress2]"
                sqltmp = sqltmp + " from DOC_TRANS_SEND where id = " + isLabel.Text

                COMMAND.CommandText = sqltmp
                COMMAND.ExecuteNonQuery()

                sqltmp = "update DOC_TRANS_SEND set doc_status = '" + status + "'"
